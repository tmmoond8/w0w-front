...
function shuffle(
  ...
  const switchingMap = (
    Math.random().toString(syllables.length).split('.').pop() as string
  )
    .split('')
    .map((s) => Number.parseInt(s));
  const arraySwitch = (from: number, to: number) => {
    const tmp = syllables[from];
    syllables[from] = syllables[to];
    syllables[to] = tmp;
  };

  syllables.forEach((_, i) => {
    arraySwitch(i, switchingMap[i]);
  });
